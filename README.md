...
```
Для работы системы логирования создайте новый проект на [сайте Rollbar](https://rollbar.com/). Получите токен и определите переменную окружения `ROLLBAR_TOKEN`.
В файл `.env` добавьте код:
```sh
ROLLBAR_TOKEN=<Ваш токен>
```
Если вы хотите, чтобы в сообщения об ошибках в Rollbar отправлялась информация об окружении, то определите переменную окружения `ROLLBAR_ENV`. В файл `.env` добавьте код:
```sh
ROLLBAR_ENV=<Имя окружения>
```

Настраивать систему логирования не обязательно, но это поможет вам быстрее находить ошибки в коде.

Также в переменных окружения необходимо указать настройки для доступа к базе данных `PostgreSQL` в формате:

```commandline
postgres://USER:PASSWORD@HOST:PORT/NAME
```
где `USER` — имя пользователя, `PASSWORD` — пароль, `HOST` — адрес сервера, `PORT` — порт, `NAME` — имя базы данных.
